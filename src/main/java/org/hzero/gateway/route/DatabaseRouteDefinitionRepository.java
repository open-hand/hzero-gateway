package org.hzero.gateway.route;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import org.apache.commons.lang3.StringUtils;
import org.hzero.common.HZeroService;
import org.hzero.gateway.route.entity.ServiceRoute;
...
/**
 * 1.1.0 采用监听spring事件拉全部路由的方式(监听到路由刷新事件时重新拉一次路由)
 * RoadMap 无需拉全部路由，通过管理端口暴露接口，允许动态增删路由
 *
 * @author XCXCXCXCX
 */
public class DatabaseRouteDefinitionRepository implements RouteDefinitionRepository {
    ...
    private List<RouteDefinition> requestAdmin() {
        LOGGER.info("fetch routes from {}", env.resolvePlaceholders(HZeroService.Admin.NAME));
        try {
            return doRequest();
        } catch (Throwable e) {
    ...
    private List<RouteDefinition> doRequest() {
        ...
        List<ServiceInstance> instances = discoveryClient.getInstances(env.resolvePlaceholders(HZeroService.Admin.NAME));
    ...
    private String getContextPath(ServiceInstance instance) {
        return instance.getMetadata() == null ?
                null : instance.getMetadata().get(CONTEXT);
    }
